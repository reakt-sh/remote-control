...
class ConnectionTracker:
    ...
    def register_remote_control(self, remote_control_id: str) -> ConnectionCapability:
        if remote_control_id not in self.capabilities:
            ...
            logger.info(f"Connection: Registered new remote control {remote_control_id}")
        return self.capabilities[remote_control_id]

    def unregister_remote_control(self, remote_control_id: str):
        if remote_control_id in self.capabilities:
            del self.capabilities[remote_control_id]
            logger.info(f"Connection: Unregistered remote control {remote_control_id}")

    def update_webtransport_status(self, remote_control_id: str, available: bool):
        capability = self.register_remote_control(remote_control_id)
